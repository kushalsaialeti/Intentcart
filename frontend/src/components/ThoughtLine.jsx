    ...
    <>
      {glyph !== 'none' ? (
        <span ref={glyphRef} className="thought-line__glyph" aria-hidden="true">
          {glyph === 'sparkle' ? (
            <Sparkles className="w-full h-full text-amber-500" strokeWidth={2} />
          ) : glyph === 'dot' ? (
      ...
      {showTimer ? (
        <span ref={timerRef} className="thought-line__timer text-xs text-[#737373] ml-1.5" data-done={isWorking ? undefined : ''} aria-hidden="true">
          0.0s
        </span>
      ) : null}
      {collapsible ? (
        <span className="thought-line__chevron ml-1" data-on={hasTrace ? '' : undefined} aria-hidden="true">
        ...
        <div className="thought-line__trace" data-open={open ? '' : undefined} aria-hidden={!open}>
          <div className="thought-line__fold">
            <div className="thought-line__steps">
              {steps.map((text, i) => {
                const done = !isWorking || i < activeStepIdx;
                const active = isWorking && i === activeStepIdx;
                return (
                  <div key={`${i}-${text}`} className="thought-line__step" data-done={done ? '' : undefined}>
                    <span className="thought-line__mark" aria-hidden="true">
                      {done ? (
                        ...
                        <i className="thought-line__pulse text-amber-500" />
                      ) : (
                        <span className="w-1.5 h-1.5 rounded-full bg-[#D4D4D0]" />
                      )}
                    </span>
                    <span className={`thought-line__step-text ${active ? 'text-[#171717] font-semibold' : 'text-[#737373]'}`}>
                      {text}
                    </span>
                  </div>
                );
